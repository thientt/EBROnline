...
namespace EBROnline.Controllers
{
    ...
    public class ProbeStatusController : AppController
    {
        /// <summary>
        ...
        {
            List<MSTProbeStatusDto> items = new List<MSTProbeStatusDto>();
        ...
        public async Task<JsonResult> Create(MSTViewModel model)
        {
            ...
            {
                MSTProbeStatusDto ass = new MSTProbeStatusDto()
                {
                    Name = model.Name,
                ...
                EBR_MST_ProbeStatus assDB = new EBR_MST_ProbeStatus();
                ConvertToWay<MSTProbeStatusDto, EBR_MST_ProbeStatus>.ConvertTo(ass, out assDB);
                var result = await ProbeStatusRep.AddAsync(assDB);
                switch (result)
                {
        ...
        {
            MSTProbeStatusDto ass = new MSTProbeStatusDto();
            ConvertToWay<EBR_MST_ProbeStatus, MSTProbeStatusDto>.ConvertTo(ProbeStatusRep.Single(id), out ass);
        ...
        public async Task<JsonResult> Edit(int id, MSTViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    EBR_MST_ProbeStatus assDB = new EBR_MST_ProbeStatus();
                    ConvertToWay<MSTProbeStatusDto, EBR_MST_ProbeStatus>.ConvertTo(ass, out assDB);
                    var result = await ProbeStatusRep.UpdateAsync(assDB);
                    switch (result)
                    {
                        case Model.SaveResult.SUCCESS:
                            Response.StatusCode = (int)HttpStatusCode.OK;
        ...
        public async Task<JsonResult> DeleteConfirmed(int id)
        {
            ...
            var result = await ProbeStatusRep.DeleteByAsync(id);
            switch (result)
            {
                case Model.SaveResult.SUCCESS:
                    Response.StatusCode = (int)HttpStatusCode.OK;
        ...
        /// <summary>
        /// Gets or sets the probe status rep.
        /// </summary>
        /// <value>
        /// The probe status rep.
        /// </value>
        [Inject]
